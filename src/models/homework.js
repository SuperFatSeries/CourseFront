import { queryCourseList, queryCourseDetail,queryCourseInfo,downloadCourseware,queryCoursewareList,queryHomeworkDetail,queryHomeworkList,queryHomeworkSubmitList,postHomeworkSubmit,queryFakeList, removeFakeList, addFakeList, updateFakeList } from '@/services/api';
...
export default {
  ...
  effects: {
    ...
    *queryHomeworkDetail({ payload }, { call, put }){
      const response = yield call(queryHomeworkDetail,payload);
      yield put({
        type: 'queryData',
        payload: response.data,
      });
    ...
    *queryHomeworkSubmitList({ payload }, { call, put }){
      const response = yield call(queryHomeworkSubmitList,payload);
      console.log(response.data.content);
      yield put({
        type: 'querySubmitList',
        payload: Array.isArray(response.data.content) ? response.data.content : [],
      });
    },
    *postHomeworkSubmit({ payload }, { call, put }) {
      const response = yield call(postHomeworkSubmit, payload); // post
      yield put({
        type: 'submitResponse',
        payload: response,
      });
